...
const flowAssistant = addKeyword("ASSISTANT").addAction(
	async (ctx, { flowDynamic, provider, state }) => {
		...
		const message_id = imagePath
			? await createMessageImage(message, imagePath, theread_id)
			: await createMessage(message, theread_id);
		...
		if (audioPath) {
			...
			return await provider.sendAudio(
				ctx.key.remoteJid,
				"src/audio/" + nameFile
			);
		} else {
